...
import dip22.ex2.Ex2Main.weatherDataFrameWithYear
...
import org.apache.spark.sql.functions.{array, asc, col, countDistinct, current_timestamp, date_format, desc, explode, max, mean, min, sum, udf, year}
...
object Ex2Main extends App {
  ...
  val weatherDataFrame: DataFrame = spark.read.format("csv").option("header", "true").load("/home/sayhelloxd/IdeaProjects/exercises-scala/ex2/data/rdu-weather-history.csv")
  ...
  val minTemp: Double = weatherDataFrame.agg(min("temperaturemin")).head().getString(0).toDouble
  val maxTemp: Double = weatherDataFrame.agg(min("temperaturemax")).head().getString(0).toDouble
  ...
  val weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn("year", year(weatherDataFrame("date")))
  weatherDataFrameWithYear.printSchema()
  val weatherDataFrameWithYearSample: Array[Row] = weatherDataFrameWithYear.take(3)
  weatherDataFrameWithYearSample.foreach(println)


  printTaskLine(5)
  // Task 5: Find min and max temperature for each year
  val aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(min("temperaturemin"), max("temperaturemax"))
  ...
  val task6DF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(countDistinct("*").alias("count"), mean("avgwindspeed").alias("avr_wind_speed"))
